...
@section('header_subtitle', 'Perbarui data pegawai')
...
<div class="animate-fade-in max-w-2xl">
    ...
    <div class="bg-white rounded-2xl shadow-card overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100">
            <h2 class="text-lg font-heading font-bold text-text">Form Edit Pegawai</h2>
            <p class="text-sm text-text-muted mt-1">Perbarui data pegawai di bawah ini</p>
        </div>

        <form action="{{ route('admin.pegawai.update', $pegawai->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <select id="skpd_id" 
                        ...
                        @foreach($skpd as $s)
                            <option value="{{ $s->id }}" {{ old('skpd_id', $pegawai->skpd_id) == $s->id ? 'selected' : '' }}>{{ $s->kode_skpd }} - {{ $s->nama_skpd }}</option>
                        @endforeach
                    </select>
                    @error('skpd_id')
                    <p class="mt-2 text-sm text-error flex items-center gap-1">
