...
<div class="mx-auto max-w-4xl space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm">
        ...
        <a href="{{ route('user.pengajuan.show', $pengajuan) }}" class="text-text-muted hover:text-primary transition-colors">{{ $pengajuan->nomor_pengajuan }}</a>
        <span class="text-text-muted">/</span>
    ...
    <div class="bg-warning/10 border border-warning/20 rounded-xl p-4 flex items-start gap-3">
        <svg class="w-5 h-5 text-warning flex-shrink-0 mt-0.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <div class="bg-white rounded-2xl shadow-card overflow-hidden">
        <div class="p-6 border-b border-gray-100">
            ...
            <p class="text-sm text-text-muted mt-1">Lengkapi dokumen yang diperlukan untuk melanjutkan proses pengajuan.</p>
        </div>

        <form action="{{ route('user.pengajuan.upload.store', $pengajuan) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div class="space-y-4">
                ...
                @endphp
                <div class="bg-surface rounded-xl p-4 border border-gray-100" data-persyaratan-id="{{ $persyaratan->id }}">
                    <!-- Header -->
                    <div class="flex items-start justify-between mb-3">
                        <div class="flex items-start gap-2">
                            <svg class="w-5 h-5 text-primary flex-shrink-0 mt-0.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    <div class="mb-3 space-y-2">
                        ...
                        <div class="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-100">
                            ...
                            <div class="flex items-center gap-2">
                                <a href="{{ $file->url }}" target="_blank" class="p-2 text-text-muted hover:text-primary transition-colors" title="Lihat File">
                    ...
                    <div class="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center hover:border-primary/50 transition-colors cursor-pointer bg-white"
                         id="dropzone-{{ $persyaratan->id }}"
                         data-persyaratan-id="{{ $persyaratan->id }}">
                        <input type="file" 
                               name="files[{{ $persyaratan->id }}]" 
                               id="file_input_{{ $persyaratan->id }}" 
                               class="hidden" 
                               accept=".pdf,.jpg,.jpeg,.png">
                        <svg class="w-8 h-8 text-text-muted mx-auto mb-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                <a href="{{ route('user.pengajuan.show', $pengajuan) }}" class="px-6 py-3 text-sm font-medium text-text-muted hover:text-text transition-colors">
                    Batal
                </a>
                <button type="button" onclick="submitForm('upload_only')" class="px-6 py-3 bg-gray-100 text-text text-sm font-semibold rounded-xl hover:bg-gray-200 transition-colors">
                    Upload
                </button>
                <button type="button" onclick="submitForm('upload_and_submit')" class="px-6 py-3 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-dark transition-colors shadow-lg shadow-primary/20">
                    Kirim Ulang
                </button>
            </div>
        </form>
